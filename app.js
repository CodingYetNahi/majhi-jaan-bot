(() => {
  ...
  function learn(text) {
    ...
    if (detectedGender) {
      ...
      return `Got it ❤️ I’ll remember that you’re ${genderLabel(detectedGender)}. I’ll use a ${opposite} companion voice for myself. You can change this anytime by saying “I am male”, “I am female”, or “I am non-binary”.`;
    }
  ...
  function localReply(text) {
    ...
    if (/^(hi|hello|hey|hii+|namaskar|नमस्कार|हाय)\b/u.test(q)) {
      if (state.userGender === 'unknown') {
        return `Hii${name} ❤️ ${voice.mrHere}. Tell me anything. For the vice-versa companion voice, you can say “I am male” or “I am female”.`;
      }
    ...
    if (/what are you doing|wyd|काय करतेस|काय करतोस/u.test(q)) {
      return `${voice.mrListen}${name} ❤️ आत्ता तुझ्याशीच बोलते/बोलतोय. तू काय करतोयस/करतेयस?`;
    }

    if (/good morning|gm\b|शुभ सकाळ/u.test(q)) {
      return `Good morning${name} ❤️ आजचा दिवस छान जावो. उठल्यावर पहिलं काय केलंस/केलंस?`;
    }

    if (/good night|gn\b|शुभ रात्री/u.test(q)) {
      return `Good night${name} ❤️ शांत झोप. उद्या पुन्हा गप्पा मारूया.`;
    }
    ...
    if (/hungry|food|जेव|भूक/u.test(q)) {
      return `अरे/अगं${name} 😄 मग आधी काहीतरी खाऊन घे. आज काय खायचा mood आहे?`;
    }

    if (/thank|thanks|धन्यवाद|thx/u.test(q)) return `Always${name} ❤️`;

    if (/who are you|तू कोण/u.test(q)) {
      return `I’m Majhi Jaan — your ${voice.en}. I can remember a few things locally, follow recent chat context, and fetch public internet information when you ask factual questions.`;
    }
